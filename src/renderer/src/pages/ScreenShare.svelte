<script>
    ...
    onMount(async () => {
        ...
        const roombaseInfo = await request.get(ChatUrls.shareScreenGetInfo());
        if (roombaseInfo.error !== 0) {
            showTips(roombaseInfo.error_msg);
            return;
        }
        globalState.channel = roombaseInfo.data.cid;

        console.log('globalState.channel----', globalState.channel, roombaseInfo);

        const webinfo = await request.get(ChatUrls.getWebInfo(globalState.channel));
