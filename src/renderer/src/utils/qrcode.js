...
QRCodeModel.prototype = {
    ...
    setupPositionProbePattern: function(row, col) { for (let r = -1; r <= 7; r++) { if (row + r <= -1 || this.moduleCount <= row + r) continue; for (let c = -1; c <= 7; c++) { if (col + c <= -1 || this.moduleCount <= col + c) continue; if ((0 <= r && r <= 6 && (c == 0 || c == 6)) || (0 <= c && c <= 6 && (r == 0 || r == 6)) || (2 <= r && r <= 4 && 2 <= c && c <= 4)) { this.modules[row + r][col + c] = true; } else { this.modules[row + r][col + c] = false; } } } },
    ...
    setupTypeInfo: function(test, maskPattern) {
        ...
        for (let i = 0; i < 15; i++) { let mod = (!test && ((bits >> i) & 1) == 1); if (i < 6) { this.modules[i][8] = mod; } else if (i < 8) { this.modules[i + 1][8] = mod; } else { this.modules[this.moduleCount - 15 + i][8] = mod; } }
...
QRRSBlock.getRSBlocks = function(typeNumber, errorCorrectLevel) {
    ...
    let list = [];
    for (let i = 0; i < length; i++) { let count = rsBlock[i * 3 + 0]; let totalCount = rsBlock[i * 3 + 1]; let dataCount = rsBlock[i * 3 + 2]; for (let j = 0; j < count; j++) { list.push(new QRRSBlock(totalCount, dataCount)); } }
    return list;
};
...
QRBitBuffer.prototype = {
    get: function(index) { let bufIndex = Math.floor(index / 8); return ((this.buffer[bufIndex] >>> (7 - index % 8)) & 1) == 1; },
        ...
        for (let row = 0; row < nCount; row++) {
            for (let col = 0; col < nCount; col++) {
                if (oQRCode.isDark(row, col)) {
                    let child = makeSVG("use", { "x": String(row), "y": String(col) });
 ...
 */
export const QRCode = function(el, vOption) {
    this._htOption = {
        width: 256,
        height: 256,
        typeNumber: 4,
